...
INVENTORY = {SHRIMP: 5, RICE: 10,
             NORI: 10, ROE: 10,
             SALMON: 5, UNAGI: 5}
ORDERING_COMPLETE = {SHRIMP: None, RICE: None, NORI: None, ROE: None, SALMON: None,
                     UNAGI: None}  # unix timestamp when an ordered ingredient will have arrived
...
def getOrders():
    ...
    for orderType in (ALL_ORDER_TYPES):
        allOrders = pyautogui.locateAllOnScreen(imPath('%s_order.png' % orderType),
...
def makeOrder(orderType):
    ...
    while time.time() < ROLLING_COMPLETE and pyautogui.locateOnScreen(imPath('clear_mat.png'), region=(
    GAME_REGION[0] + 115, GAME_REGION[1] + 295, 220, 175)) is None:
        time.sleep(0.1)
...
def findAndClickPlatesOnBelt():
    ...
    for color in ('pink', 'blue', 'red'):
        result = pyautogui.locateCenterOnScreen(imPath('%s_plate_color.png' % (color)),
...
def orderIngredient(ingredient):
    ...
    if ingredient == RICE and ORDERING_COMPLETE[RICE] is None:
        ...
        if pyautogui.locateOnScreen(imPath('cant_afford_rice.png'),
        ...
        if pyautogui.locateOnScreen(imPath('cant_afford_%s.png' % (ingredient)),
